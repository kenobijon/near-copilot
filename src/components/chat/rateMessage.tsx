...
function RateMessage({ id }: Props) {
    const [messageRating, setMessageRating] = useState<MessageRating>();

    const rateResponse = async (data: MessageRating) => {
        setMessageRating(data);
        const response = await rateMessage(data);

        if (!response) setMessageRating(undefined);
    };
